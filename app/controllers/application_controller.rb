...
class ApplicationController < ActionController::Base
  ...
  def get_section_highlight(id)
    ...
    # section = Section.find(id)
    @article = Article.where(articable_id: id, highlight: true, created_at: (Date.today - 1.month).beginning_of_month..(Date.today - 1.month).end_of_month).order(updated_at: "DESC").first
  end
